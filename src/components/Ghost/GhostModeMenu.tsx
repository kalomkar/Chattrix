      ...
      >
        <div className="p-5 border-b border-white/10 flex items-center justify-between bg-white/5">
          <div className="flex items-center space-x-3 text-blue-400">
            <Ghost size={24} />
            <h2 className="text-lg font-bold text-white">GB Chattrix Settings</h2>
          </div>
        ...
        <div className="p-6 space-y-6 max-h-[70vh] overflow-y-auto no-scrollbar">
          <section className="space-y-4">
             <h3 className="text-white/30 text-[10px] font-bold uppercase tracking-widest pl-1">Privacy / Ghost Mode</h3>
             <ToggleItem 
                icon={<EyeOff size={18} />} 
          ...
          <section className="space-y-4">
             <div className="flex items-center justify-between pl-1">
                <h3 className="text-white/30 text-[10px] font-bold uppercase tracking-widest">Auto Reply</h3>
                <button 
                  onClick={() => setAutoReply(prev => ({ ...prev, enabled: !prev.enabled }))}
                  className={cn("w-10 h-5 rounded-full relative transition-all duration-300", autoReply.enabled ? "bg-blue-600" : "bg-white/10")}
                >
                  <div className={cn("absolute top-1 w-3 h-3 rounded-full bg-white transition-all duration-300 shadow-sm", autoReply.enabled ? "right-1" : "left-1")} />
                </button>
             </div>
             {autoReply.enabled && (
                <textarea 
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white placeholder-white/20 focus:border-blue-500/50 outline-none transition-all"
                  rows={3}
                />
             )}
          </section>
...
function ToggleItem({ icon, label, description, active, onClick }: { icon: React.ReactNode, label: string, description: string, active: boolean, onClick: () => void }) {
  return (
    <div className="flex items-center justify-between group cursor-pointer p-3 rounded-2xl bg-white/5 border border-white/5 hover:border-white/10 transition-all" onClick={onClick}>
      <div className="flex items-center space-x-4">
        <div className={cn("p-2.5 rounded-xl transition-all", active ? "bg-blue-600/20 text-blue-400" : "bg-white/5 text-white/40")}>
          {icon}
        </div>
      ...
      <div className={cn("w-10 h-5 rounded-full relative transition-all duration-300", active ? "bg-blue-600" : "bg-white/10")}>
        <div className={cn("absolute top-1 w-3 h-3 rounded-full bg-white transition-all duration-300 shadow-sm", active ? "right-1" : "left-1")} />
      </div>
    </div>
  );
}
